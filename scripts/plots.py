...
if __name__ == "__main__":
    ...
    def compile_df_plt(dfss, value_types, value_names):
        ...
        ixs_indiv = np.logical_and(
            df_plt["t_type"] == "ICR", df_plt["r_type"] == "individualized"
        )
        ...
        with sns.plotting_context("talk"):
            ...
            for ii in range(len(g2.axes.flat)):
                ...
                if not "cost" in title:
                    ax.set_yscale(
                        "function",
                        functions=[lambda x: x**2, lambda z: np.power(z, 1 / 2)],
                    )
                    ax.set_ylim(*ylim)
                    ax.set_yticks(ticks)
    ...
    res_ce = (
        dfs.loc[dfs["t_type"] == "CE", :].groupby(["scm"]).mean().reset_index().round(2)
    )
