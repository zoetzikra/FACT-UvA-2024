...
class GenericSCM(StructuralCausalModel):
    ...
    def __init__(
        ...
        super(GenericSCM, self).__init__(
            dag, u_prefix=u_prefix, costs=costs, y_name=y_name
        )
    ...
    def _mcmc_mixed(
        ...
        **kwargs
        ...
        nuts_kernel = numpyro.infer.MixedHMC(
            numpyro.infer.HMC(model, trajectory_length=1.2)
        )
        mcmc = numpyro.infer.MCMC(
            nuts_kernel,
            num_samples=num_samples,
    ...
    def _abduct_node_par_mcmc(
        ...
        def model(x_pa, x_j=None):
            ...
            x_j = numpyro.sample(
                node, dist.Normal(input, GenericSCM.SMALL_VAR), obs=x_j
            )
        ...
        if type_dist is dist.Normal or type_dist is torch.distributions.Normal:
            ...
            type_dist is dist.Categorical
            or type_dist is torch.distributions.Categorical
        ):
    ...
    def _abduct_node_obs_discrete(self, node, obs):
        ...
        def log_posterior_y(y):
            ...
            denominator = jnp.log(
                jnp.exp(log_prob_joint(1)) + jnp.exp(log_prob_joint(0))
            )
    ...
    def _abduct_node_obs_discrete_pyro(self, node, obs, nr_samples=1000, temperature=1):
        ...
        x_ch_pa_dict = {}
        for ch in self.model[node]["children"]:
            x_ch_dict[ch] = torch.tensor(
                obs_df[[ch]].to_numpy().flatten(), dtype=torch.float64
            )
            x_ch_pa_dict[ch] = torch.tensor(
                obs_df_dummy[list(self.model[ch]["parents"])].to_numpy(),
                dtype=torch.float64,
            )
        ...
        def model_binary(x_pa, x_ch_dict=None, x_ch_pa_dict=None):
            ...
            x_j = pyro.sample(
                node,
                pyro.distributions.Bernoulli(probs=input),
            ...
            for ch_ix in range(len(chs)):
                ...
                d_ch = pyro.distributions.Normal(
                    x_ch_input + u_ch_dist.loc, u_ch_dist.scale
                )
        ...
        for jj in range(nr_samples):
            samples.append(
                xj_posterior_model(
                    x_pa, x_ch_dict=x_ch_dict, x_ch_pa_dict=x_ch_pa_dict
                )[0].numpy()
            )
    ...
    def _abduct_node_obs_mcmc(
            ...
            if ch in chs_infer:
                ...
                if isinstance(self.model[ch]["noise_distribution"], dist.Normal):
                    ...
                    raise NotImplementedError(
                        "only normal distibrution supported so far"
                    )
        ...
        mixing_dist = dist.Categorical(
            probs=np.array([1.0 - d_node.probs, d_node.probs])
        )
    ...
    def predict_log_prob_obs(self, x_pre, y_name, y=1, **kwargs):
        ...
        p = d.probs
        if (isinstance(p, torch.Tensor) or isinstance(p, jnp.ndarray)) and len(
            p.shape
        ) > 0:
            p = p.item()
        p = torch.tensor(p)
    ...
    def predict_log_prob_individualized_obs(
        ...
        for ch in self.model[y_name]["children"]:
            ...
            x_ch_pa_post = obs_df_dummy_post[
                list(scm_post.model[ch]["parents"])
            ].to_numpy()
        ...
        def log_joint_prob_ys(ys):
            ...
            ys = jnp.array(ys)
            log_probs_chs = [
                d_chs[ch].log_prob(val_chs[ch], ys=ys) for ch in d_chs.keys()
            ]
    ...
    def predict_log_prob_individualized_obs_pyro(
        ...
        x_pa_pre = torch.tensor(
            obs_df_pre[list(self.model[y_name]["parents"])].to_numpy()
        )
        x_pa_post = torch.tensor(
            obs_df_post[list(scm_post.model[y_name]["parents"])].to_numpy()
        )
