...
class StructuralCausalModel:
    ...
    def __init__(
        self, dag: DirectedAcyclicGraph, costs=None, y_name=None, u_prefix="u_"
    ):
        """
        Args:
    ...
    def get_values(self, var_names=None):
        ...
        if len(var_names) > 0:
            arr = torch.stack(
                [self.model[node]["values"] for node in var_names], dim=1
            ).numpy()
    ...
    def get_noise_values(self):
        ...
        df = pd.DataFrame(
            arr,
            columns=[self.name_to_uname(var_name) for var_name in self.dag.var_names],
        )
        return df
    ...
    def get_noise_distributions(
        self, check_deterministic=False, return_intervened=False
    ):
        """
    ...
    def sample_context(self, size: int, seed=None):
        ...
        for node in self.topological_order:
            ...
            if d is None:
                ...
                self.model[node]["noise_values"] = self.model[node][
                    "noise_distribution"
                ].repeat(size)
    ...
    def do(self, intervention_dict, verbose=False):
        ...
        for node in intervention_dict.keys():
            ...
            scm_itv.model[node]["noise_distribution"] = torch.tensor(
    ...
    def abduct_node(
        self, node, obs, scm_partially_abducted=None, infer_type="mcmc", **kwargs
    ):
        """Abduction
        ...
        if node in obs.index and self.INVERTIBLE:
            ...
            # if not all parents observed then the noise is dependent on the unobserved parent
                ...
                return self._abduct_node_par_unobs(
                    node, obs, scm_partially_abducted, **kwargs
                )
    ...
    def compute(self, do={}):
        ...
        for node in self.topological_order:
            if node in do.keys():
                self.model[node]["values"] = torch.tensor(do[node]).repeat(
