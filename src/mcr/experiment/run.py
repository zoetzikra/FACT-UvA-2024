...
def run_experiment(scm_name, N, N_recourse, gamma, thresh, lbd, savepath, use_scm_pred=False, iterations=5,
    ...
    while existing_runs < iterations:
        ...
        print('Creating {} batches of data with {} observations'.format(N_BATCHES, batch_size))
        ...
        if n_recourse_contendors < N_recourse:
            print(f"not enough recourse contendors ({n_recourse_contendors}). try again")
            n_fails += 1
            if n_fails >= 30:
        ...
        print('Fitting {} models for multiplicity robustness assessment.'.format(nr_refits_batch0))
        model_refits_batch0 = []
