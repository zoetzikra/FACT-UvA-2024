...
def recourse_population(
    ...
    if t_type == "improvement":
        ...
        ixs_causes = np.array(
            [
                np.arange(len(intv_features))[cause == intv_features][0]
                for cause in causes
            ]
        )
    ...
    logging.debug(
        "Iterating through {} individuals to suggest recourse...".format(
            len(ixs_recourse)
        )
    )
    flag=False
    ...
    if (
        r_type == "individualized"
        and t_type == "improvement"
        and predict_individualized
    ):
    ...
    stats["costs"] = list(
        costs
    )  # costs for the interventions (list with len(X.columns) indexes)
    stats["lbd"] = float(lbd)
